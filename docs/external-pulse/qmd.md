...
- **Displacement — overlap with existing decisions, no replacement:**
  ...
  - **Skill packaging:** interlinked already ships `skills/enforce/SKILL.md` and installs it into `.claude/skills/`, `.codex/skills/`, `.interlinked/skills/`. qmd's "the CLI installs its own skill from the npm package" is the same pattern at a slightly different surface (per-project + `~/.agents/skills/`). Parity, not displacement.
