...
describe("checkBroadObjectTypes", () => {
	...
	// FP refinement (139-repo audit, 2026-05): generator output uses
	// broad types extensively by design. Supermodel's sdk/DefaultApi.ts
	// (auto-generated by OpenAPI Generator) produced 290 strong_typing
	// FPs in one file. The fix is to change the generator config; we
	// MUST NOT flag the file.

	it("does NOT flag generator-output (OpenAPI Generator header)", () => {
		const code = [
			"/* tslint:disable */",
			"/* eslint-disable */",
			"/**",
		...
		expect(checkBroadObjectTypes(code, "sdk/apis/DefaultApi.ts")).toEqual([]);
	});

	it("does NOT flag protobuf-generated headers", () => {
		const code = [
			"// Code generated by protoc-gen-go. DO NOT EDIT.",
			"export type ProtoMessage = Record<string, any>;",
		].join("\n");
		expect(checkBroadObjectTypes(code, "proto/foo.ts")).toEqual([]);
	});

	it("does NOT flag @generated headers (Relay/codegen)", () => {
		const code = [
			"// @generated SignedSource<<...>>",
			"// Do not modify by hand.",
			"export type Schema = Record<string, any>;",
		].join("\n");
		expect(checkBroadObjectTypes(code, "src/__generated__/Schema.ts")).toEqual([]);
	});

	// Positive cases — real hand-written code MUST still fire.

	it("STILL flags hand-written code with `Record<string, any>`", () => {
		const code = [
			"// Application config types.",
			"export type AppConfig = Record<string, any>;",
		].join("\n");
		expect(checkBroadObjectTypes(code, "src/types.ts").length).toBe(1);
	});

	it("STILL flags `as Function` in hand-written code", () => {
		const code = "const handler = userInput as Function;";
		expect(checkBroadObjectTypes(code, "src/handlers.ts").length).toBe(1);
	});

	it("STILL flags index signatures in real source", () => {
		const code = "export type Cache = { [k: string]: any };";
		expect(checkBroadObjectTypes(code, "src/cache.ts").length).toBe(1);
	});
});
