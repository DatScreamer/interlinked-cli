...
export function renderOpenCodeBridgeSource(binaryPath: string): string {
	return [
		...
		'    return Boolean(env.OPENCODE2) || env.INTERLINKED_CLIENT === "opencode2" || String(env.XDG_CONFIG_HOME || "").includes("opencode-v2") || argv.some((part) => String(part).includes("opencode2"));',
		"}",
