...
//
// WHY THIS FILE DUPLICATES ../generic.integration.test.ts (2026-07-31):
// the per-edit mutation runner (scratch/two-box-runner/runner.mjs
// `testScopeFor`) selects the test suite for a measurement by EXACT STEM
// match: mutating `generic.ts` loads only `<dir>/generic.test.ts` or
// `<dir>/__tests__/generic.test.ts` — never a sibling `generic.integration.
// test.ts`. That thorough mocked suite is therefore invisible to mutation
// measurement even though it exercises this exact module (tracked as a
// runner-infra gap, not fixed here — see project task "Mutation runner scopes
// tests to the EXACT-STEM file only"). This file is the one the runner
// actually loads, so the deterministic mocked-boundary suite lives HERE too.
// The original six smoke tests are kept (unweakened, same names/assertions)
// but rebuilt on the same mocks — the earlier real-`npx`-spawning versions
// measured as Stryker "Timeout" mutants on the static `ESLINT_CONFIG_FILES`
// array, almost certainly from a cold `npx` package-resolution stall in the
// runner's sandboxed worktree.
//
...
//
// `outputParsers` below is the SAME real (unmocked) module — imported as a
// namespace so a handful of tests can `vi.spyOn` one parser to assert it was
// (or was NOT) invoked, then `.mockRestore()`. That is required for a class of
// mutant at the `(result.stdout || "").trim(); if (!output) return [];` sites:
// every one of the six parsers below catches its own JSON.parse failure and
// returns `[]`/`null` internally, so a mutant that (a) replaces the `""`
// fallback with a non-empty literal, or (b) neuters the `if (!output)` guard,
// produces the SAME return value as the unmutated code (the parser is called
// with garbage and quietly swallows it) — indistinguishable by return-value
// assertions alone. The spy asserts the real discriminator: whether the
// parser was called at all, not just what the caller ultimately returned.
...
// ===========================================================================
// Original smoke-test suite (retained verbatim by name/assertion; rebuilt on
// the mocks above instead of a real tmp dir + real subprocess spawns so the
// mutation runner's scoped measurement of this file is deterministic).
// ===========================================================================

describe("generic tool runners", () => {
	it("runEslint returns [] when there's no eslint config in the tree", () => {
		expect(runEslint(input(projectScope()))).toEqual([]);
	});
	...
	it("runOxlint returns an array (empty when oxlint is not installed)", () => {
		...
		expect(Array.isArray(runOxlint(input(projectScope())))).toBe(true);
	});

	it("runKnip returns an array", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: null, error: enoent() }));
		expect(Array.isArray(runKnip(input(projectScope())))).toBe(true);
	});

	it("runSemgrep returns an array", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: null, error: enoent() }));
		expect(Array.isArray(runSemgrep(input(projectScope())))).toBe(true);
	});

	it("runGitleaks returns an array", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: null, error: enoent() }));
		expect(Array.isArray(runGitleaks(input(projectScope())))).toBe(true);
	});
...
// ===========================================================================
// findEslintConfig boundary coverage — the 5-level walk and the 9 recognized
// config filenames are internal to generic.ts (not exported), so both are
// exercised indirectly through runEslint by checking whether spawnSync gets
// invoked (config found) or not (config missing).
// ===========================================================================

describe("findEslintConfig boundary coverage", () => {
	// Every filename findEslintConfig recognizes. Kept as an independent
	// literal (not imported from source) so each one is a real assertion
	// about the CONTRACT, not a mirror of the source's own list.
	const CONFIG_NAMES = [
		".eslintrc.json",
		".eslintrc.js",
		".eslintrc.cjs",
	...
	it.each(CONFIG_NAMES)("finds a config file named %s at the project root", (name) => {
		// findEslintConfig walks from scope.projectRoot, not from targetFile's
		// directory — the config file must exist AT projectRoot (hop 0).
		existsSyncMock.mockImplementation(existsForPaths([`${PROJECT_ROOT}/${name}`]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 0 }));
		runEslint(input(fileScope()));
		expect(spawnSyncMock).toHaveBeenCalledTimes(1);
	});

	it("does NOT walk a 6th directory level (5-level cap, exclusive boundary)", () => {
		// startDir has 6 hops before the fs root: hop0..hop4 are checked (5
		// levels, matching findEslintConfig's `i < 5`); hop5 is one level
		// PAST the cap and must never be probed. A config planted only at
		// hop5 must therefore not be found.
		const startDir = "/x0/x1/x2/x3/x4/x5";
		existsSyncMock.mockImplementation(existsForPaths(["/x0/.eslintrc.json"]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 0 }));
		const out = runEslint(input(projectScope({ projectRoot: startDir })));
		expect(out).toEqual([]);
		expect(spawnSyncMock).not.toHaveBeenCalled();
	});

	it("DOES find a config at hop4 (5-level cap, inclusive boundary)", () => {
		const startDir = "/y0/y1/y2/y3/y4";
		existsSyncMock.mockImplementation(existsForPaths(["/y0/.eslintrc.json"]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 0 }));
	...
	it("stops walking once dirname reaches a fixpoint (fs root) instead of repeating for all 5 iterations", () => {
		// "/a" -> dirname -> "/" -> dirname("/") === "/" (fixpoint). The walk
		// must stop there (2 levels checked: "/a" then "/") rather than
		// re-probing "/" for the remaining 3 iterations of the `i < 5` loop.
		existsSyncMock.mockReturnValue(false);
		runEslint(input(projectScope({ projectRoot: "/a" })));
		expect(existsSyncMock.mock.calls.length).toBe(9 * 2);
	});
});
...
describe("runEslint (sync)", () => {
	...
	it("reads diagnostics from stderr too (stdout+stderr concat path)", () => {
		...
		// The stdout fallback must contribute nothing (not a placeholder string) —
		// otherwise it corrupts the FIRST line's file field via concatenation
...
describe("runOxlint (sync)", () => {
	...
	it("short-circuits to [] on ENOENT even if status/stdout would otherwise look like a real run", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: oxlintJson() }));
		expect(runOxlint(input(fileScope()))).toEqual([]);
	});
	...
	it("returns [] on non-zero status with undefined stdout (the '' fallback)", () => {
		// Sharper than a return-value check alone: parseOxlintJson swallows
		// invalid JSON internally and also returns [], so a mutant that (a)
		// replaces the `""` fallback with a non-empty literal, or (b) neuters
		// the `if (!output)` guard, would STILL return [] here — it would just
		// get there by calling the parser with garbage instead of skipping it.
		// Asserting the parser was never invoked is the actual discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseOxlintJson");
	...
	it("strips a leading BOM before parsing (a '.trim()' skip would leave it and fail JSON.parse)", () => {
		// A bare, non-JSON-whitespace prefix like U+FEFF is stripped by JS's
		// `.trim()` but NOT tolerated by `JSON.parse` — verified empirically
		// (JSON.parse("﻿{}") throws; JSON.parse("﻿{}".trim()) does
		// not). Without `.trim()`, this BOM-prefixed-but-otherwise-valid
		// payload would fail to parse and silently collapse to [], masking
		// real findings.
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: `﻿${oxlintJson()}` }));
		const out = runOxlint(input(fileScope()));
		expect(out).toHaveLength(2);
		expect(out[0]?.ruleId).toBe("eslint(eqeqeq)");
	});
...
describe("runKnip (sync)", () => {
	...
	it("short-circuits to [] on ENOENT even if status/stdout would otherwise look like a real run", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: knipJson() }));
		expect(runKnip(input(fileScope()))).toEqual([]);
	});

	it("treats a non-ENOENT spawn error as a real (non-zero) run and parses output", () => {
		spawnSyncMock.mockReturnValue(
			spawnResult({ status: 1, stdout: knipJson(), error: Object.assign(new Error("t"), { code: "ETIMEDOUT" }) }),
		);
		expect(runKnip(input(projectScope()))).toHaveLength(2);
	});
	...
	it("returns [] on non-zero status with empty stdout (trim → '' guard)", () => {
		// Same double-defense discriminator as runOxlint above: parseKnipJson
		// also swallows invalid JSON and returns [], so return-value alone
		// can't tell "guard fired, parser skipped" from "guard neutered,
		// parser called with garbage and quietly failed."
		const parseSpy = vi.spyOn(outputParsers, "parseKnipJson");
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: undefined }));
		expect(runKnip(input(fileScope()))).toEqual([]);
		expect(parseSpy).not.toHaveBeenCalled();
		parseSpy.mockRestore();
	...
	it("does NOT filter in project mode even if targetFile + filterToFile are set", () => {
		// mode !== "file" must guard the whole `&&` chain — a stray targetFile
		// with filterToFile set must not trigger filtering outside file mode.
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: knipJson() }));
		const out = runKnip(input(projectScope({ targetFile: "src/app.ts", filterToFile: true })));
		expect(out).toHaveLength(2);
	});
...
describe("runSemgrep (sync)", () => {
	...
	it("short-circuits to [] on ENOENT even if status/stdout would otherwise look like a real run", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: semgrepJson() }));
		expect(runSemgrep(input(fileScope()))).toEqual([]);
	});

	it("treats a non-ENOENT spawn error as a real (non-zero) run and parses output", () => {
		spawnSyncMock.mockReturnValue(
			spawnResult({ status: 1, stdout: semgrepJson(), error: Object.assign(new Error("t"), { code: "ETIMEDOUT" }) }),
		);
		expect(runSemgrep(input(fileScope()))).toHaveLength(1);
	});
	...
	it("does not call parseSemgrepJson when stdout is undefined (the '' fallback keeps the trim empty)", () => {
		// Unlike the whitespace case above (a truthy stdout that merely trims
		// to ""), an undefined stdout also exercises the `result.stdout || ""`
		// fallback literal itself. parseSemgrepJson swallows invalid JSON and
		// returns [] too, so a mutant that replaces the fallback with a
		// non-empty literal (or neuters the `if (!output)` guard) would still
		// return [] here — asserting the parser was never called is the
		// actual discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseSemgrepJson");
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: undefined }));
		expect(runSemgrep(input(fileScope()))).toEqual([]);
		expect(parseSpy).not.toHaveBeenCalled();
		parseSpy.mockRestore();
	...
	it("strips a leading BOM before parsing (a '.trim()' skip would leave it and fail JSON.parse)", () => {
		...
		const out = runSemgrep(input(fileScope()));
		expect(out).toHaveLength(1);
		expect(out[0]?.ruleId).toBe("rules.no-eval");
	});
...
describe("runGitleaks (sync)", () => {
	...
	it("short-circuits to [] on ENOENT even if status/stdout would otherwise look like real leaks", () => {
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: gitleaksJson() }));
		expect(runGitleaks(input(projectScope()))).toEqual([]);
	});
	...
	it("treats a non-ENOENT spawn error as a real (non-zero) run and parses output", () => {
		...
		expect(runGitleaks(input(projectScope()))).toHaveLength(1);
	});

	it("treats FTL appearing in stdout (not just stderr) as fatal — combinedOutput draws from both streams", () => {
		// Valid JSON that would otherwise parse to a real leak, but the
		// (fake) filename happens to contain the fatal marker "FTL". If
		// combinedOutput only drew from stderr (or dropped stdout's real
		// content), this fatal marker would be invisible and the function
		// would fall through to parsing it as a genuine finding instead.
		const stdoutWithFatalMarker = JSON.stringify([
			{ File: "srFTLc/secrets.ts", StartLine: 8, RuleID: "aws-access-key", Description: "AWS Access Key detected" },
		]);
		spawnSyncMock.mockReturnValue(
			spawnResult({ status: 1, stderr: "plain stderr, no marker", stdout: stdoutWithFatalMarker }),
		);
		expect(runGitleaks(input(projectScope()))).toEqual([]);
	});
	...
	it("returns [] when status === 1 with empty stdout (trim → '' guard, non-fatal)", () => {
		// stdout: "" is itself falsy, so this ALSO exercises the `|| ""`
		// fallback (not just the guard). parseGitleaksJson swallows invalid
		// JSON and returns [] too, so a mutant that replaces the fallback
		// literal (or neuters the `if (!output)` guard) would still return []
		// here — asserting the parser was never called is the discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseGitleaksJson");
...
describe("runDepAudit (sync)", () => {
	...
	it("osv-scanner short-circuits to null on ENOENT even with a promising status/stdout", () => {
		hasOsvScannerMock.mockReturnValue(true);
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: osvJson() }));
		// no package.json → overall null after fall-through (ENOENT must not
		// be masked by a status/stdout shape that would otherwise parse).
		expect(runDepAudit(input(projectScope()))).toBeNull();
	});

	it("osv-scanner treats a non-ENOENT spawn error as a real run and still parses", () => {
		// The ENOENT check must key off error.code, not error presence alone —
		// a non-ENOENT error (e.g. a spawn timeout) must fall through to a
		// real parse rather than being swallowed as "tool not installed".
		hasOsvScannerMock.mockReturnValue(true);
		spawnSyncMock.mockReturnValue(
			spawnResult({
				status: 1,
		...
		expect(runDepAudit(input(projectScope()))?.tool).toBe("osv-scanner");
	});

	it("osv-scanner treats a non-ENOENT spawn error as a real (status===1) run and parses output", () => {
		hasOsvScannerMock.mockReturnValue(true);
		spawnSyncMock.mockReturnValue(
			spawnResult({ status: 1, stdout: osvJson(), error: Object.assign(new Error("t"), { code: "ETIMEDOUT" }) }),
		);
		expect(runDepAudit(input(projectScope()))?.tool).toBe("osv-scanner");
	});
	...
	it("osv-scanner status === 1 with undefined stdout → null (the '' fallback)", () => {
		...
		// parseOsvScannerJson swallows invalid JSON and returns null too, so a
		// mutant that replaces the fallback literal (or neuters the
		// `if (!output)` guard) would still return null here — asserting the
		// parser was never called is the actual discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseOsvScannerJson");
		hasOsvScannerMock.mockReturnValue(true);
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: undefined }));
		expect(runDepAudit(input(projectScope()))).toBeNull();
		expect(parseSpy).not.toHaveBeenCalled();
	...
	it("osv-scanner strips a leading BOM before parsing (a '.trim()' skip would fail JSON.parse)", () => {
		hasOsvScannerMock.mockReturnValue(true);
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: `﻿${osvJson()}` }));
		expect(runDepAudit(input(projectScope()))?.tool).toBe("osv-scanner");
	});
	...
	it("npm audit short-circuits to null on ENOENT even with a promising status/stdout", () => {
		hasOsvScannerMock.mockReturnValue(false);
		existsSyncMock.mockImplementation(existsForPaths([PACKAGE_JSON]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, error: enoent(), stdout: npmAuditJson() }));
		expect(runDepAudit(input(projectScope()))).toBeNull();
	});

	it("npm audit treats a non-ENOENT spawn error as a real run and still parses", () => {
		hasOsvScannerMock.mockReturnValue(false);
		existsSyncMock.mockImplementation(existsForPaths([PACKAGE_JSON]));
		spawnSyncMock.mockReturnValue(
			spawnResult({
	...
	it("npm audit treats a non-ENOENT spawn error as a real run and parses output", () => {
		hasOsvScannerMock.mockReturnValue(false);
		existsSyncMock.mockImplementation(existsForPaths([PACKAGE_JSON]));
		spawnSyncMock.mockReturnValue(
			spawnResult({
		...
		expect(runDepAudit(input(projectScope()))?.tool).toBe("npm audit");
	});
	...
	it("npm audit undefined stdout → null (the '' fallback)", () => {
		...
		// parseNpmAuditJson swallows invalid JSON and returns null too, so a
		// mutant that replaces the fallback literal (or neuters the
		// `if (!output)` guard) would still return null here — asserting the
		// parser was never called is the actual discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseNpmAuditJson");
		hasOsvScannerMock.mockReturnValue(false);
		existsSyncMock.mockImplementation(existsForPaths([PACKAGE_JSON]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: undefined }));
		expect(runDepAudit(input(projectScope()))).toBeNull();
	...
	it("npm audit strips a leading BOM before parsing (a '.trim()' skip would fail JSON.parse)", () => {
		hasOsvScannerMock.mockReturnValue(false);
		existsSyncMock.mockImplementation(existsForPaths([PACKAGE_JSON]));
		spawnSyncMock.mockReturnValue(spawnResult({ status: 1, stdout: `﻿${npmAuditJson()}` }));
		expect(runDepAudit(input(projectScope()))?.tool).toBe("npm audit");
...
describe("runOxlintAsync", () => {
	...
	it("returns [] on a non-zero code with empty stdout (trim → '' guard)", async () => {
		// parseOxlintJson swallows invalid JSON and returns [] too, so a
		// mutant that neuters the `if (!output)` guard would still return []
		// here — asserting the parser was never called is the discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseOxlintJson");
	...
	it("strips a leading BOM before parsing (a '.trim()' skip would leave it and fail JSON.parse)", async () => {
		runProcessAsyncMock.mockResolvedValue(procResult({ code: 1, stdout: `﻿${oxlintJson()}` }));
		const out = await runOxlintAsync(input(fileScope()));
		expect(out).toHaveLength(2);
		expect(out[0]?.ruleId).toBe("eslint(eqeqeq)");
...
describe("runKnipAsync", () => {
	...
	it("returns [] on a non-zero code with empty stdout (trim → '' guard)", async () => {
		// parseKnipJson swallows invalid JSON and returns [] too, so a mutant
		// that neuters the `if (!output)` guard would still return [] here —
		// asserting the parser was never called is the discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseKnipJson");
		runProcessAsyncMock.mockResolvedValue(procResult({ code: 1, stdout: "" }));
		expect(await runKnipAsync(input(fileScope()))).toEqual([]);
		expect(parseSpy).not.toHaveBeenCalled();
	...
	it("does NOT filter in project mode even if targetFile + filterToFile are set", async () => {
		runProcessAsyncMock.mockResolvedValue(procResult({ code: 1, stdout: knipJson() }));
		const out = await runKnipAsync(
			input(projectScope({ targetFile: "src/app.ts", filterToFile: true })),
		);
		expect(out).toHaveLength(2);
	});
...
describe("runSemgrepAsync", () => {
	...
	it("returns [] on a clean run with empty stdout (trim → '' guard)", async () => {
		// parseSemgrepJson swallows invalid JSON and returns [] too, so a
		// mutant that neuters the `if (!output)` guard would still return []
		// here — asserting the parser was never called is the discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseSemgrepJson");
...
describe("runGitleaksAsync", () => {
	...
	it("returns [] when code === 1 with empty stdout (trim → '' guard)", async () => {
		// parseGitleaksJson swallows invalid JSON and returns [] too, so a
		// mutant that neuters the `if (!output)` guard would still return []
		// here — asserting the parser was never called is the discriminator.
		const parseSpy = vi.spyOn(outputParsers, "parseGitleaksJson");
