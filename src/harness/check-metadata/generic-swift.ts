// Metadata fragment: Swift / iOS checks. Mirrors `check-registry/entries-swift.ts`.
// Seven of these wrap the previously-orphan swift.ts detectors; the other
// thirteen are new in this rollout. Determinism follows the same classifier
// convention used elsewhere — pattern-anchored regexes are `fully_deterministic`,
// scope-tracked or pairing-heuristic checks are `heuristic`. Composed into
// GENERIC_CHECK_META in ./generic.ts.

import type { CheckMeta } from "./types.js";

export const GENERIC_SWIFT_META: Record<string, CheckMeta> = {
	// === Swift / iOS ===
	// Mirrors `check-registry/entries-swift.ts`. Seven of these wrap the
	// previously-orphan swift.ts detectors; the other thirteen are new in
	// this rollout. Determinism follows the same classifier convention used
	// elsewhere — pattern-anchored regexes are `fully_deterministic`,
	// scope-tracked or pairing-heuristic checks are `heuristic`.
	swift_task_detached: {
