...
describe("detectCjsInEsm — CommonJS used in an ES module", () => {
	// ── positives: must fire ──────────────────────────────────────────────────
	it("flags require() in a file that also imports (the vitest case)", () => {
...
`;
	...
	it("flags module.exports in an ESM file", () => {
...
`;
	...
	it("flags require() in a .mjs file even with no import", () => {
		expect(detectCjsInEsm(`const cfg = require("./config.json");\n`, "/repo/build.mjs").length).toBe(1);
	});

	it("flags bare __dirname in an ESM file with no import.meta", () => {
...
`;
	...
	it("does NOT flag a pure CJS file (no import/export anchor)", () => {
...
`;
	...
	it("does NOT flag test files — the runner injects require/__dirname", () => {
		const src = `import { it } from "vitest";
...
`;
	...
	it("does NOT flag require( inside comments or string literals", () => {
		const src = `import { x } from "./x.js";
...
`;
	...
	it("does NOT flag __dirname under the import.meta.dirname dual pattern", () => {
...
`;
	...
	it("does NOT flag a file using createRequire (sanctioned ESM escape hatch)", () => {
...
`;
	...
	it("does NOT flag @codegen-data hook-template carriers", () => {
