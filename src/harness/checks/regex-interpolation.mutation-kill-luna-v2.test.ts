...
describe("regex_from_interpolation mutation survivors", () => {
	// test-contract: only RegExp with at most twenty spaces after `new` is a call site.
	it("requires the complete bounded constructor spacing", () => {
		expect(run("const re = new RegExp(`${user}`);")).toHaveLength(1);
	});

	// test-contract: a plain assignment from an escape call exempts the identifier, but a later reassignment cancels it.
	...
	// test-contract: an empty template substitution body is dynamic, while a nonempty CONST_CASE substitution is exempt.
	...
	// test-contract: trailing whitespace after a bare template is harmless, but a chained expression is outside the template shape.
	it("requires a bare template after the closing backtick", () => {
		expect(run("const re = new RegExp(`${user}`   );")).toHaveLength(1);
		expect(run("const re = new RegExp(`${user}`.trim());")).toHaveLength(0);
	});

	// test-contract: empty operands and ordinary literal operands are classified distinctly from dynamic operands.
	...
	// test-contract: malformed template scans conservatively and do not produce a finding.
	it("bails on malformed templates", () => {
		expect(run("const re = new RegExp(`${user);")).toHaveLength(0);
	});

	// test-contract: malformed interpolation classification is conservative, while a valid dynamic interpolation reports.
	...
	// test-contract: a regex literal's final slash and flags hide embedded RegExp text from call-site matching.
	it("lexes regex literals through their flags", () => {
		expect(run("const hidden = /RegExp(`${user}`)/giu;")).toHaveLength(0);
	});

	// test-contract: a call at the exact end of source is parsed without reading beyond the input.
	it("handles end-of-input call boundaries", () => {
		expect(run("new RegExp(`${user}`)")).toHaveLength(1);
	});

	// test-contract: a division slash after a value exposes the following real RegExp call, unlike a regex-literal slash.
	...
	// test-contract: plus signs inside strings, regexes, and nested expressions do not split the outer argument.
	...
	// test-contract: quoted strings containing comment markers remain string content, and comments containing calls remain inert.
