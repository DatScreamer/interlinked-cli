...
//   - Uses the bundled `typescript` compiler API (already a devDep of the
//     CLI; no new runtime dependency).
//   - Builds a single-file Program per call. The CLI dispatches per file
//     and the compiler API has no shared "create once, reuse across files"
//     handle that fits the InlineMatch[] return contract, so we accept the
//     per-file cost. Bounded by file size (≤1000 lines) and AST pre-scan
//     for AsExpression nodes.
//   - When the type-checker can't resolve a type (parse errors, missing
//     dependencies, generic constraint contexts), the detector skips
//     silently — false-firing on broken type info is worse than missing a
//     real case.
//
 ...
 */
function targetIsAnyOrUnknownSyntax(typeNode: ts.TypeNode): boolean {
...
function collectSmugglingCasts(
	...
	const visit = (node: ts.Node): void => {
		...
		if (ts.isAsExpression(node) || ts.isTypeAssertionExpression(node)) {
			...
			if (targetIsAnyOrUnknownSyntax(typeNode)) {
				ts.forEachChild(node, visit);
				return;
			}
			...
			if (isSmugglingCast(checker, sourceType, targetType)) {
				const { line } = ts.getLineAndCharacterOfPosition(
					sourceFile,
					node.getStart(sourceFile),
				);
...
export function checkTypeSmuggling(content: string, filePath: string): InlineMatch[] {
	...
	try {
		const built = createSingleFileProgram(content, filePath);
		if (!built) return [];
		result = collectSmugglingCasts(built.program, built.sourceFile);
	} catch {
