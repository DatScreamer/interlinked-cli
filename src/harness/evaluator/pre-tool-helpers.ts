...
// them back. The cohesive Bash/Read check-blocks at the bottom are factored
// out of the orchestrator body as pure functions that take explicit
// parameters and return their result; the call sites in `evaluatePreToolUse`
// invoke them at the same position and handle the result identically, so the
// first-match-wins sequencing is unchanged.
