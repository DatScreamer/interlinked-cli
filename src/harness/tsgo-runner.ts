 ...
 */
const PASS_COMPLETE_RE = /^(?:build finished in\b|Found \d+ errors?\b)/i;
/** ANSI escape sequences (incl. tsgo's `[2J[3J[H` screen-clear). */
...
function stripWatchTimestamp(line: string): string {
 ...
 */
function findTsconfigDir(path: string): string | null {
 ...
 */
function buildInfoPath(projectRoot: string, tag: "watch" | "cold"): string {
 ...
 */
function filterDiagnosticsForFile(
